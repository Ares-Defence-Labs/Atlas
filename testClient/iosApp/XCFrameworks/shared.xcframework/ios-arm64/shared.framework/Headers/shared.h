...
__attribute__((swift_name("ApplicationScope")))
@interface SharedApplicationScope : SharedBase <SharedCoroutineScope>
+ (instancetype)alloc __attribute__((unavailable));
+ (instancetype)allocWithZone:(struct _NSZone *)zone __attribute__((unavailable));
+ (instancetype)applicationScope __attribute__((swift_name("init()")));
@property (class, readonly, getter=shared) SharedApplicationScope *shared __attribute__((swift_name("shared")));
- (void)cancel __attribute__((swift_name("cancel()")));
- (id<SharedJob>)launchCPUBlock:(id<SharedKotlinSuspendFunction1>)block __attribute__((swift_name("launchCPU(block:)")));
- (id<SharedJob>)launchIOBlock:(id<SharedKotlinSuspendFunction1>)block __attribute__((swift_name("launchIO(block:)")));
@property (readonly) id<SharedKotlinCoroutineContext> coroutineContext __attribute__((swift_name("coroutineContext")));
@property SharedCrashPolicy *crashPolicy __attribute__((swift_name("crashPolicy")));
@property void (^crashReporter)(SharedKotlinThrowable *) __attribute__((swift_name("crashReporter")));
@end
...
__attribute__((objc_subclassing_restricted))
__attribute__((swift_name("CrashPolicy")))
@interface SharedCrashPolicy : SharedKotlinEnum<SharedCrashPolicy *>
+ (instancetype)alloc __attribute__((unavailable));
+ (instancetype)allocWithZone:(struct _NSZone *)zone __attribute__((unavailable));
- (instancetype)initWithName:(NSString *)name ordinal:(int32_t)ordinal __attribute__((swift_name("init(name:ordinal:)"))) __attribute__((objc_designated_initializer)) __attribute__((unavailable));
@property (class, readonly) SharedCrashPolicy *none __attribute__((swift_name("none")));
@property (class, readonly) SharedCrashPolicy *cancelScope __attribute__((swift_name("cancelScope")));
@property (class, readonly) SharedCrashPolicy *rethrow __attribute__((swift_name("rethrow")));
+ (SharedKotlinArray<SharedCrashPolicy *> *)values __attribute__((swift_name("values()")));
@property (class, readonly) NSArray<SharedCrashPolicy *> *entries __attribute__((swift_name("entries")));
@end
...
- (void)bootstrapVmFromNavEngine __attribute__((swift_name("bootstrapVmFromNavEngine()")));
- (void)onAfterAppearing __attribute__((swift_name("onAfterAppearing()")));
...
/**
 * Fetches a registered service from the AtlasContainer (with class string type)
 */
...
@interface SharedCFlow (Extensions)
- (SharedCFlow<id> *)broadcastCoroutineScope:(id<SharedCoroutineScope>)coroutineScope __attribute__((swift_name("broadcast(coroutineScope:)")));
- (SharedCFlow<id> *)debounceCoroutineScope:(id<SharedCoroutineScope>)coroutineScope timeInMillis:(int64_t)timeInMillis __attribute__((swift_name("debounce(coroutineScope:timeInMillis:)")));
- (SharedCFlow<id> *)retryAttemptsCoroutineScope:(id<SharedCoroutineScope>)coroutineScope attempts:(int64_t)attempts __attribute__((swift_name("retryAttempts(coroutineScope:attempts:)")));
- (SharedCFlow<id> *)throttleLastCoroutineScope:(id<SharedCoroutineScope>)coroutineScope timeInMillis:(int64_t)timeInMillis __attribute__((swift_name("throttleLast(coroutineScope:timeInMillis:)")));
@end
...
__attribute__((objc_subclassing_restricted))
__attribute__((swift_name("GuidanceKt")))
@interface SharedGuidanceKt : SharedBase
+ (id<SharedDeferred>)asyncContext:(id<SharedKotlinCoroutineContext>)context start:(SharedCoroutineStart *)start block:(id<SharedKotlinSuspendFunction1>)block __attribute__((swift_name("async(context:start:block:)"))) __attribute__((unavailable("'async' can not be called without the corresponding coroutine scope. Consider wrapping 'async' in 'coroutineScope { }', using 'runBlocking { }', or using some other 'CoroutineScope'")));
+ (id<SharedJob>)launchContext:(id<SharedKotlinCoroutineContext>)context start:(SharedCoroutineStart *)start block:(id<SharedKotlinSuspendFunction1>)block __attribute__((swift_name("launch(context:start:block:)"))) __attribute__((unavailable("'launch' can not be called without the corresponding coroutine scope. Consider wrapping 'launch' in 'coroutineScope { }', using 'runBlocking { }', or using some other 'CoroutineScope'")));
@end
